...
v0.4.1 uses deterministic provenance artifacts for release and downstream agent
...
```powershell
uv run pic provenance create --schema-dir schemas --sbom-ref dist\percolation-inversion-compiler-0.4.1.sbom.json --artifact-ref dist\percolation_inversion_compiler-0.4.1-py3-none-any.whl --artifact-ref dist\percolation_inversion_compiler-0.4.1.tar.gz --output provenance.json
```
...
```powershell
uv run pic sbom create --format pic --output dist\percolation-inversion-compiler-0.4.1.sbom.json
uv run pic sbom create --format cyclonedx --output dist\percolation-inversion-compiler-0.4.1.cyclonedx.json
```

The PIC-SBOM format is retained for backward compatibility. CycloneDX JSON is
the standard release SBOM for v0.4.1. SBOM output is an audit input, not a
