...
def test_release_version_metadata_is_consistent() -> None:
    ...
    assert citation["version"] == "0.4.3"
    assert citation["date-released"] == "2026-06-20"
    assert re.search(r"^__version__\s*=\s*[\"']0\.4\.3[\"']", init_text, re.MULTILINE)
    assert re.search(r"^## v0\.4\.3 - 2026-06-20$", changelog, re.MULTILINE)
    assert changelog.index("## v0.4.3 - 2026-06-20") < changelog.index("## v0.4.2")
...
def test_release_checklist_warns_against_local_dist_star_publish() -> None:
    ...
    assert "dist\\percolation_inversion_compiler-0.4.3-py3-none-any.whl" in text
    assert "dist\\percolation_inversion_compiler-0.4.3.tar.gz" in text
